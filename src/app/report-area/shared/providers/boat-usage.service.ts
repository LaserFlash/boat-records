...
export class BoatUsageService {
  ...
  public lastMonthEachBoat: BehaviorSubject<
    { boat: string; duration: number }[]
  > = new BehaviorSubject([]);
  public usageTimes: BehaviorSubject<{ boat: string; duration: number }[]> =
    new BehaviorSubject([]);
  ...
  constructor(private db: AngularFirestore, BOATS: KnownBoatsService) {
    this.itemsCollection = db.collection<UsageInfo>("/boatUsage", (ref) =>
      ref.orderBy("endTime", "desc").orderBy("boatID")
    );
    this.itemsCollection.valueChanges().subscribe((data) => {
      ...
      const seen = {};
      this.lastEachBoat.next(
        this.sortedByDate.filter((usage) => {
      ...
      recordsInMonth.forEach((usage) => {
        if (!totals[usage.boatID]) {
          totals[usage.boatID] = 0;
        }
  ...
  getBatch(offset, batch_size) {
    return this.db
      .collection<UsageInfo>("/boatUsage", (ref) =>
        ref.orderBy("endTime", "desc").startAfter(offset).limit(batch_size)
      )
      .snapshotChanges()
      .pipe(
