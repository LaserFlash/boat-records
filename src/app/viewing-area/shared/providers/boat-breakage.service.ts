...
export class BoatBreakageService {
  constructor(private db: AngularFirestore) {
    this.itemsCollectionBroken = db.collection<BreakageInfo>(
      "/boatBreakages",
      (ref) => ref.orderBy("timestamp", "desc")
    );
    this.itemsCollectionFixed = db.collection<BreakageInfo>(
      "/boatBreakagesFixed",
      (ref) => ref.orderBy("timestampFixed", "desc")
    );
    ...
    this.itemsData.subscribe((val) => {
      this.buildBreakages(val, this.items);
    });
    this.itemsData.subscribe((val) => {
      this.buildBreakages(val, this.original);
    });
    this.recentThreeItems = db
      .collection<BreakageInfo>("/boatBreakages", (ref) =>
        ref.orderBy("timestamp", "desc").limit(3)
      )
      .valueChanges();

    this.recentThreeItems.subscribe((val) => {
      this.buildBreakages(val, this.recentItems);
    });
    this.fixedItemsData = this.itemsCollectionFixed.valueChanges();
    this.fixedItemsData.subscribe((val) => {
      this.buildBreakages(val, this.fixedItems);
    });
    this.fixedItemsData.subscribe((val) => {
      this.buildBreakages(val, this.fixedItemsOriginal);
    });
  }
